...
def updateChecklistArray(data,char_name):
    if list(__user_postit__).index(char_name) == 14:
        clu_ofs = getSectionOffsets(data)[1] + 0x32C
        for i in range(12):
            current_ofs = clu_ofs + i * 4
            data = alterInt(data, current_ofs, __user_postit__[char_name][i])
            if i == 8:
                clu_ofs += 0x4
            if i == 9:
                clu_ofs += 0x37C
            if i == 10:
                clu_ofs += 0x84
    elif list(__user_postit__).index(char_name) > 14:
        clu_ofs = getSectionOffsets(data)[1] + 0x31C
        for i in range(12):
            current_ofs = clu_ofs + list(__user_postit__).index(char_name) * 4 + i * 19 * 4
            data = alterInt(data, current_ofs, __user_postit__[char_name][i])
            if i == 8:
                clu_ofs += 0x4C
            if i == 9:
        ...
        clu_ofs = getSectionOffsets(data)[1] + 0x6C
        for i in range(12):
            current_ofs = clu_ofs + list(__user_postit__).index(char_name) * 4 + i * 14 * 4
            data = alterInt(data, current_ofs, __user_postit__[char_name][i])
            if i == 5:
                clu_ofs += 0x14
            if i == 8:
...
def checklistAllCharacters(user_achievements):
    for i in __characters_achievements__.keys():
        for j in __characters_achievements__.get(i):
            if j in user_achievements:
                __user_postit__[i].append(2)
            else:
                __user_postit__[i].append(0)
        if __user_postit__[i][8]==2:
            del __user_postit__[i][7]
        elif __user_postit__[i][7]==2:
            __user_postit__[i][8]=1
            del __user_postit__[i][7]
        else:
            __user_postit__[i][8]=0
            del __user_postit__[i][7]
    return

def updatePostIt(data, user_achievements):
    checklistAllCharacters(user_achievements)

    if _debug:
        for char, post_it_array in __user_postit__.items():
            if char:
        ...
        for char in __user_postit__.keys():
            if char:
                # print(f"Updating {char}'s post-it note")
                data = updateChecklistArray(data, char)
            else:
